...
/**
 * @version v1.0
 * @author frogchou Zxing的一个管理类，用于扩展Zxing的一些功能。
 */
public class ZxingHandler {
	 ...
	 * 
	 * @param matrix
	 *            输入一个已有的BitMatrix 对象，这个对象中已经存放了一个二维码
	 ...
	 *            指定输出的目标文件
	 */
	public static void writeToFileWithColor(BitMatrix matrix, String format, File file) throws IOException {
	...
	public static void createEPSQRCode(Settings setting, File file, String content) {
		...
		EPSGraphics2D funcOld = new EPSGraphics2D(point_x, point_y, setting.getQrcodeSize() * blockSize, setting.getQrcodeSize() * blockSize);
		try {
			ExportQrCode.fill2VectorLine(funcOld, GetBitMatrix(content, setting.getQrcodeSize(), setting.getQrcodeErrorRate()), blockSize);
	...
	public static void createPDFQRCode(Settings setting, File file, String content) {
		double point_x = 0;
		double point_y = 0;
		float codeSize = setting.getQrcodeSize() * setting.getBlockSize();
		PDFGraphics2D funcOld = new PDFGraphics2D(point_x, point_y, codeSize, codeSize);
		try {
			ExportQrCode.fill2Vector(funcOld, GetBitMatrix(content, setting.getQrcodeSize(), setting.getQrcodeErrorRate()), setting.getBlockSize());
			PrintStream psFile = new PrintStream(file);
			psFile.append(funcOld.toString());
			psFile.close();
		} catch (WriterException e) {
	...
	public static void createPDFQRCode(Settings setting, File file, String content, String title) {
		double point_x = 0;
		double point_y = 0;
		float codeSize = setting.getQrcodeSize() * setting.getBlockSize();

		FontMetrics fm = new JLabel().getFontMetrics(setting.getFont());
		int titleHeight = fm.getHeight();
		int titleWidth = fm.stringWidth(title);

		PDFGraphics2D funcOld = new PDFGraphics2D(point_x, point_y, titleWidth, codeSize + titleHeight);
		funcOld.setFont(setting.getFont());
		funcOld.setFontRendering(FontRendering.VECTORS);

		try {
			ExportQrCode.fill2Vector(funcOld, GetBitMatrix(content, setting.getQrcodeSize(), setting.getQrcodeErrorRate()), title, titleWidth, titleHeight, setting.getBlockSize());
			PrintStream psFile = new PrintStream(file);
			psFile.append(funcOld.toString());
			psFile.close();
		} catch (WriterException e) {
	...
	public static void createPDFQRCodeForPAHW(File file, String content, String title) {
		...
		try {
			...
			qrcodeSize = bitMatrix.getWidth() * blockSize;

			PDFGraphics2D funcOld = new PDFGraphics2D(0, 0, totalWidth, totalHeight);
			funcOld.setFont(font);
			funcOld.setFontRendering(FontRendering.VECTORS);

			ExportQrCode.fill2VectorForPAHW1(funcOld, bitMatrix, totalWidth, totalHeight, title, titleWidth, titleHeight, blockSize);
			PrintStream psFile = new PrintStream(file);
			psFile.append(funcOld.toString());
			psFile.close();
		} catch (WriterException e) {
	...
	public static void createSVGQRCode(Settings setting, File file, String content) {
		...
		double point_y = 0;
		final int blockSize = 1;
		SVGGraphics2D funcOld = new SVGGraphics2D(point_x, point_y, setting.getQrcodeSize() * blockSize, setting.getQrcodeSize() * blockSize);
		try {
			ExportQrCode.fill2VectorLine(funcOld, GetBitMatrix(content, setting.getQrcodeSize(), setting.getQrcodeErrorRate()), blockSize);
			PrintStream psFile = new PrintStream(file);
			psFile.append(funcOld.toString());
	...
	public static BitMatrix GetBitMatrixForPAHW(String content, int width, int height, ErrorCorrectionLevel errorCorrectionLevel) throws WriterException {
		BitMatrix bitMatrix = null;
		MultiFormatWriter multiFormatWriter = new MultiFormatWriter();
		Hashtable<EncodeHintType, Object> hints = new Hashtable<EncodeHintType, Object>();
		hints.put(EncodeHintType.CHARACTER_SET, "UTF-8");
		hints.put(EncodeHintType.MARGIN, 0); // 控制码图白边
		hints.put(EncodeHintType.ERROR_CORRECTION, errorCorrectionLevel); // 容错率
