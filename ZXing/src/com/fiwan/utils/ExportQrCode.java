...
public class ExportQrCode {
	 ...
	 * @throws IOException
	 */
	public static void doExport(BitMatrix bitMatrix, QCodeFileType type, double width_height, java.io.OutputStream out) throws WriterException, IOException {
	...
	/**
	 * @param bitMatrix
	 *            注意如果是矢量数据,width不要太大容易造成文件过大
	 * @param type
	 * @param out
	 * @throws WriterException
	 * @throws IOException
	 */
	public static void doExport(BitMatrix bitMatrix, QCodeFileType type, MatrixToImageConfig config, java.io.OutputStream out) throws WriterException, IOException {
		double point_x = 0;
		double point_y = 0;
		...
		case PDF:
			...
			break;
		case SVG:
			v = new SVGGraphics2D(point_x, point_y, (width) * blockSize + width, (height) * blockSize + height);
			break;
		default:
			break;
		}
	 ...
	 * @throws WriterException
	 */
	public static void fill2Vector(VectorGraphics2D g, BitMatrix bitMatrix, float blockSize) throws WriterException {
		...
		if (true) {
			for (int x = 0; x < width; x++) {
				float theX = x * blockSize;
				for (int y = 0; y < height; y++) {
					if (bitMatrix.get(x, y)) {
						float theY = y * blockSize;
	...
	public static void fill2Vector(VectorGraphics2D g, BitMatrix bitMatrix, String title, int titleWidth, int titleHeight, float blockSize) throws WriterException {
		if (g == null || bitMatrix == null)
			return;
		...
		if (true) {
			g.drawString(title, 0, titleHeight);
			for (int x = 0; x < width; x++) {
				float theX = x * blockSize + (titleWidth - width * blockSize) / 2;
				for (int y = 0; y < height; y++) {
					if (bitMatrix.get(x, y)) {
						float theY = y * blockSize + titleHeight;
						g.fill(new Rectangle2D.Double(theX, theY, blockSize, blockSize));
					}
				}
			}
	...
	public static void fill2VectorForPAHW(VectorGraphics2D g, BitMatrix bitMatrix, int totalWidth, int totalHeight, String title, int titleWidth, int titleHeight, int blockSize) throws WriterException {
		if (g == null || bitMatrix == null)
			return;
		...
		if (true) {
			for (int x = 0; x < width; x++) {
				float theX = x * blockSize + (totalWidth - width * blockSize) / 2;
				for (int y = 0; y < height; y++) {
					if (bitMatrix.get(x, y)) {
			...
			g.drawString(title, (totalWidth - titleWidth) / 2, startY + height * blockSize + titleHeight + spacing);
		}
	}
	 ...
	 * @throws WriterException
	 */
	public static void fill2VectorForPAHW1(VectorGraphics2D g, BitMatrix bitMatrix, int totalWidth, int totalHeight, String title, int titleWidth, int titleHeight, int blockSize) throws WriterException {
		if (g == null || bitMatrix == null)
			return;
	...
	public static void fill2VectorLine(VectorGraphics2D funcOld, BitMatrix bitMatrix, int blockSize) throws WriterException {
